...
from cruds.serializers import CrudSerializer
import math
...
class Crud(generics.GenericAPIView):
    ...
    def get(self,request):
        ...
        start_num = limit_num * (page_num - 1)
        end_num = limit_num * page_num
        search_param = request.GET.get("search")
        notes = CrudModel.objects.all()
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"status": "success", "notes": serializer.data}, status= status.HTTP_201_CREATED)
        else:
            return Response({"status": "fail", "notes": serializer.errors}, status = status.HTTP_400_BAD_REQUEST)


class CrudDetail(generics.GenericAPIView):
    queryset = CrudModel.objects.all()
    serializer_class = CrudSerializer

    def get_note(self, pk):
        try:
            return CrudModel.objects.get(pk=pk)
        except:
            return None
    ...
    def patch(self, request, pk):
        ...
        if note == None:
            return Response({"status": "fail", "message": f"Note with id:{pk} note found"}, status=status.HTTP_404_NOT_FOUND)
